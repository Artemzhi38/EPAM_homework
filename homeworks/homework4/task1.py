...
If first line is a number return true if number in an
interval [1, 3)* and false otherwise.
...
Definition of done:
 ...
 - tests do a cleanup and remove remove files generated
 by tests
...
def read_magic_number(path: str) -> bool:
    if not os.path.isfile(path):
        raise ValueError('Incorrect path to the file')
    with open(path) as text:
        return 1 <= float(text.readline()) < 3
